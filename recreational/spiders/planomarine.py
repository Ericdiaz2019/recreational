...
# Generate 5 random numbers and concatenate them into a single string
...
class planomarine(scrapy.Spider):
    name = 'planomarine'  # Unique identifier for the spider
    
    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.planomarine.com/default.asp?page=xNewInventory',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            boat_company = boat_company.replace('®','').replace('™','').replace('Sueños', 'Suenos')
            if (len(boat_info) == 3):
                boat_model = 'N/A'
                boat_floor = boat_info[2]
                boat_floor = boat_floor.replace('®','').replace('™','').replace('Sueños', 'Suenos')
            else:
                boat_model = boat_info[2]
                boat_model = boat_model.replace('®','').replace('™','').replace('Sueños', 'Suenos')
                boat_floor = boat_info[3]
                boat_floor = boat_floor.replace('®','').replace('™','').replace('Sueños', 'Suenos')
            try:
                boat_location =  boats.find('li', class_='VehicleLocation').find('span', class_='InvDistance').text
            except:
                boat_location = 'Pottsboro TX'
            try:
                boat_stock = boats.find('li', class_='stockno').find('span', class_='unitValue').text
            except:
                boat_stock = random_numbers
            try:
                boat_msrp = boats.find('div', class_='dvPriceGroupContainer').find('span').text.replace('Click for Quote!', 'N/A').replace('$','').replace(',','').replace('CALL FOR PRICING','N/A').replace("SPECIAL REDUCED REBATE PRICE",'N/A').replace('Black/White','N/A').strip()
            except:
                boat_msrp = 'N/A'
            try:
                boat_discount = boats.find('div', class_='dvPriceGroup').find('span', class_='NoBGInvPrice').text.replace('Click for Quote!', 'N/A').replace('$','').replace(',','').replace('CALL FOR PRICING', 'N/A').strip()
            except:
                boat_discount = 'N/A'
            try:
            ...
            with open(f'DailyFiles/Plano Marine {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company, boat_model, boat_floor, boat_length, boat_engine, boat_stock, 'Plano Marine', boat_location, boat_msrp, boat_discount, today])
