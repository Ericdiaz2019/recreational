...
class bama(scrapy.Spider):
    name = 'bama'  # Unique identifier for the spider

    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Travel Trailer': 'https://www.bamarv.com/default.asp?condition=new&page=inventory&pg=1&subcategory=travel%20trailer',
            'FifthWheel': 'https://www.bamarv.com/default.asp?condition=new&page=inventory&pg=1&subcategory=fifth%20wheel',
            'ToyHauler': 'https://www.bamarv.com/default.asp?condition=new&page=inventory&pg=1&subcategory=toy%20hauler%20-%20fifth%20wheel&subcategory=toy%20hauler%20-%20travel%20trailer',
            'ClassC Gas': 'https://www.bamarv.com/default.asp?condition=new&page=inventory&pg=1&subcategory=class%20c',
            'ClassB Gas': 'https://www.bamarv.com/default.asp?condition=new&page=inventory&pg=1&subcategory=class%20b',
            'ClassA Gas': 'https://www.bamarv.com/default.asp?condition=new&page=inventory&pg=1&subcategory=class%20a',
            'Destination Trailer' : 'https://www.bamarv.com/default.asp?condition=new&page=inventory&pg=1&subcategory=destination'
        }
    ...
    def parse_units(self, response):
        ...
        for unit in units:
            ...
            unit_stock = unit.find('span', class_="v7list-vehicle__stock-number").find('span', class_="v7list-vehicle__stock-number-value").text

            unit_category = response.meta['category_name']
            unit_location = unit.find('span', class_="v7list-vehicle__location").find('span', class_="v7list-vehicle__location-value").text

            with open(f'DailyFiles/Bama {today}.csv', 'a', newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:  # If file is empty, write header
                    writer.writerow(['Year','Company','Brand','FloorPlan','Msrp','Discount','Stock-Number','Unit Type','Location','Dealer','Date'])
                
                writer.writerow([unit_year,unit_company,unit_brand,unit_floor,unit_msrp,unit_discount,unit_stock,unit_category,unit_location,'Bama',today])
