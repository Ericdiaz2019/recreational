...
class blmBoats(scrapy.Spider):
    name = 'blmBoats'  # Unique identifier for the spider

    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.blmboats.com/--inventory?category=boat&condition=new&pg=1',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            try:
                ...
                boat_msrp = 'N/A'
            try:
                boat_engine = boats.find('li', class_='vehicle-specs__item--engine').find('span', class_='vehicle-specs__value').text.strip()
            except:
                boat_engine = 'N/A'
            try:
            ...
            with open(f'DailyFiles/Blm Boats {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company,boat_model,boat_floor,boat_length,boat_engine,boat_stock,'Blm Boats','Madera CA',boat_msrp,boat_discount,today])
