...
class captainschoiceboats(scrapy.Spider):
    name = 'captainschoiceboats'  # Unique identifier for the spider

    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.captainschoiceboats.com/--inventory?condition=new&pg=1',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            boat_location = 'Fort Walton Beach FL'
            ...
            try:
                ...
                boat_discount = 'N/A'
            try:
                boat_stock = num() #boats.find('li', class_='vehicle-specs__item--stock-number').find('span', class_='vehicle-specs__value').text
            except:
                boat_stock = num()
            ...
            with open(f'DailyFiles/Captains choice {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company,boat_model,boat_floor,boat_length,'N/A',boat_stock,'Captains choice',boat_location,boat_msrp,boat_discount,today])
