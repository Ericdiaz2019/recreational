...
class futrellmarine(scrapy.Spider):
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            boat_company = boats.get('data-boat-make')
            boat_model = boats.get('data-boat-model')
            ...
            with open(f'DailyFiles/Futrell Marine {today}.csv', 'a',newline='') as file:
                ...
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','Floor', 'Length','Engine', 'Stock Number', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company,boat_model,'N/A',boat_length,'N/A',boat_stock,boat_location,boat_msrp,'N/A',today])
