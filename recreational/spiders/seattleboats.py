...
class settlespider(scrapy.Spider):
    name = 'settlespider'  # Unique identifier for the spider

    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.seattleboat.com/--inventory?condition=new&pg=1&sortby=length_overall%7Cas&subcategory=cruiser&subcategory=cruiser%20%28power%29',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            boat_company = boats.get('data-unit-make')
            model_raw = boats.get('data-unit-model')
            ...
            print(f'TESTING: {boat_year}, {boat_company}', {boat_model}, {boat_floor})

            with open(f'DailyFiles/SeattleBoats {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine','Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company,boat_model,boat_floor,boat_length,'N/A',boat_stock,'Seattle Boats','Seattle WA',boat_msrp,'N/A',today])
