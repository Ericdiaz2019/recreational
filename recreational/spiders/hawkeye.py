...
class hawkeye(scrapy.Spider):
    name = 'hawkeye'  # Unique identifier for the spider
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            with open(f'DailyFiles/Hawkeye {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company, boat_model, boat_floor, boat_length, boat_engine, boat_stock, 'Hawkeye', boat_location, boat_msrp, boat_discount, today])
