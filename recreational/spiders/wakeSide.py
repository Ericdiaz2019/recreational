...
class spiderwakeside(scrapy.Spider):
    name = 'spiderwakeside'  # Unique identifier for the spider
    
    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.wakesidemarine.com/default.asp?page=xNewInventory#page=xNewInventory&vc=pontoon',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            try:
                ...
                boat_discount = 'N/A'
            try:
                boat_length = boats.find('li', class_='InvLength').find('span', class_='unitValue').text.strip()
            except:
                boat_length = 'N/A'
            try:
            ...
            with open(f'DailyFiles/WakeSide {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company,boat_model,boat_floor,boat_length,boat_engine,boat_stock,'WakeSide',boat_location,boat_msrp,boat_discount,today])
