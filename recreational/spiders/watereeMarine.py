...
class watereeMarine(scrapy.Spider):
    name = 'watereeMarine'  # Unique identifier for the spider
    
    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.watereemarinainc.com/default.asp?page=xNewInventory#page=xNewInventory&p=1&vt=boat',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            with open(f'DailyFiles/Wateree Marine {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company, boat_model, boat_floor, boat_length, boat_engine, boat_stock, 'Wateree Marine', boat_location, boat_msrp, boat_discount, today])
