...
class steves(scrapy.Spider):
    name = "steves"


    def start_requests(self):
        web_links = {
            'Travel Trailer': 'https://www.stevesrv.com/rv-search?s=true&condition=1&types=29&pagesize=12',
            'FifthWheel': 'https://www.stevesrv.com/rv-search?s=true&condition=1&types=5&pagesize=12',
            'ToyHauler': 'https://www.stevesrv.com/rv-search?s=true&condition=1&types=26%2C28&pagesize=12',
        }
    ...
    def parse_units(self, response):
        ...
        for unit in units:
            ...
            with open(f'DailyFiles/Steves {today}.csv', 'a', newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:  # If file is empty, write header
                    writer.writerow(['Year','Company','Brand','FloorPlan','Msrp','Discount','Stock-Number','Unit Type','Location','Dealer','Date'])
                
                writer.writerow([unit_year,unit_company,unit_brand,unit_floor,unit_msrp,unit_discount,unit_stock,unit_category,unit_location,'Steves',today])
