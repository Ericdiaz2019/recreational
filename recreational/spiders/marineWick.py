...
class wickspider(scrapy.Spider):
    name = 'wickspider'  # Unique identifier for the spider

    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.marinesalesofpickwick.com/--inventory?condition=new&pg=1',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            with open(f'DailyFiles/MarineSales {today}.csv', 'a',newline='') as file:
                ...
                writer.writerow([boat_year, boat_company,boat_model,boat_floor,boat_length,'N/A',boat_stock,'Marine Sales','Counce TN',boat_msrp,'N/A',today])
