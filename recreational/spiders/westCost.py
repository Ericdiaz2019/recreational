...
class westCost(scrapy.Spider):
    name = 'westCost'  # Unique identifier for the spider

    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.westcoastboatcenterreno.com/--inventory?condition=new&pg=1',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            try:
                boat_msrp = boats.find('span', class_='vehicle-price--current').find('span', class_='vehicle-price__price').text.replace('$', '').replace(',', '').replace('Click for a Quote','').strip()
            except:
                boat_msrp = 'N/A'
            try:
                boat_discount = 'N/A'
            ...
            with open(f'DailyFiles/West Cost {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company,boat_model,boat_floor,boat_length,'N/A',boat_stock,'West Cost',boat_location,boat_msrp,boat_discount,today])
