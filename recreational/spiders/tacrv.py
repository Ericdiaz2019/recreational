...
class tac(scrapy.Spider):
    name = 'tac'  # Unique identifier for the spider

    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Travel Trailer': 'https://www.tacrvs.com/--inventory?condition=new&pg=1&sortby=Year%7Casc&subcategory=travel%20trailer',
            'FifthWheel': 'https://www.tacrvs.com/--inventory?condition=new&pg=1&sortby=Year%7Casc&subcategory=fifth%20wheel',
            'ToyHauler': 'https://www.tacrvs.com/--inventory?condition=new&pg=1&sortby=Year%7Casc&subcategory=toy%20hauler%20-%20fifth%20wheel&subcategory=toy%20hauler%20-%20travel%20trailer',
            'ClassC Gas': 'https://www.tacrvs.com/--inventory?condition=new&pg=1&sortby=Year%7Casc&subcategory=class%20c',
        }
    ...
    def parse_units(self, response):
        ...
        for unit in units:
            ...
            unit_location = 'Moyock NC'

            with open(f'DailyFiles/TAC {today}.csv', 'a', newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:  # If file is empty, write header
                    writer.writerow(['Year','Company','Brand','FloorPlan','Msrp','Discount','Stock-Number','Unit Type','Location','Dealer','Date'])
                
                writer.writerow([unit_year,unit_company,unit_brand,unit_floor,unit_msrp,unit_discount,unit_stock,unit_category,unit_location,'TAC',today])
