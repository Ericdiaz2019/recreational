...
class dicks(scrapy.Spider):
    name = "dicks"


    def start_requests(self):
        web_links = {
            'Travel Trailer': 'https://www.dicksrv.com/rv-search?s=true&condition=1&types=29&pagesize=12',
            'FifthWheel': 'https://www.dicksrv.com/rv-search?s=true&condition=1&types=5&pagesize=12',
            'ToyHauler': 'https://www.dicksrv.com/rv-search?s=true&condition=1&types=28&pagesize=12',
            'Destination Trailer' : 'https://www.dicksrv.com/rv-search?s=true&condition=1&pagesize=12&types=3'
        }
    ...
    def parse_units(self, response):
        ...
        for unit in units:
            ...
            with open(f'DailyFiles/Dicks {today}.csv', 'a', newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:  # If file is empty, write header
                    writer.writerow(['Year','Company','Brand','FloorPlan','Msrp','Discount','Stock-Number','Unit Type','Location','Dealer','Date'])
                
                writer.writerow([unit_year,unit_company,unit_brand,unit_floor,unit_msrp,unit_discount,unit_stock,unit_category,unit_location,'Dicks',today])
