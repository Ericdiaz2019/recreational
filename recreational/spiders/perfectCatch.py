...
class perfectCatch(scrapy.Spider):
    name = 'perfectCatch'  # Unique identifier for the spider
    
    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.perfectcatchmarine.com/default.asp?page=xNewInventory#page=xNewInventory&vt=boat',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            boat_location = 'Mountain Home AR'
            boat_category = response.meta['category_name']
            try:
                boat_stock = generate_random_numbers() #boats.find('li', class_='stockno').find('span', class_='unitValue').text
            except:
                boat_stock = generate_random_numbers()
            try:
                boat_msrp = boats.find('div', class_='dvPriceGroup').find('span', class_='InvRetPrc').text.replace('Click for Quote!', 'N/A').replace('$','').replace(',','').strip()
            except:
                boat_msrp = 'N/A'
            try:
            ...
            with open(f'DailyFiles/Perfect Catch {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company,boat_model,boat_floor,boat_length,boat_engine,boat_stock,'Perfect Catch',boat_location,boat_msrp,boat_discount,today])
