...
class ketelsen(scrapy.Spider):
    name = "ketelsen"


    def start_requests(self):
        web_links = {
            'Travel Trailer': 'https://www.ketelsenrv.com/rv-search?s=true&condition=1&types=29&pagesize=12',
            'FifthWheel': 'https://www.ketelsenrv.com/rv-search?s=true&condition=1&pagesize=12&types=5',
            'ToyHauler': 'https://www.ketelsenrv.com/rv-search?s=true&condition=1&pagesize=12&types=26%2C28',
            'Truck Camper': 'https://www.ketelsenrv.com/rv-search?s=true&condition=1&pagesize=12&types=30',
        }
    ...
    def parse_units(self, response):
        ...
        for unit in units:
            ...
            with open(f'DailyFiles/Ketelsen {today}.csv', 'a', newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:  # If file is empty, write header
                    writer.writerow(['Year','Company','Brand','FloorPlan','Msrp','Discount','Stock-Number','Unit Type','Location','Dealer','Date'])
                
                writer.writerow([unit_year,unit_company,unit_brand,unit_floor,unit_msrp,unit_discount,unit_stock,unit_category,unit_location,'Ketelsen',today])
