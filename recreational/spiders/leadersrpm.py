...
class leadersrpm(scrapy.Spider):
    name = 'leadersrpm'  # Unique identifier for the spider
    
    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.leadersrpm.com/default.asp?page=xAllInventory&s=Year&d=A#page=xAllInventory&p=1&vt=boat&t=new',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            if (len(boat_info) == 3):
                ...
                boat_floor = boat_floor.replace('®','').replace('™','')
            boat_location = 'Kalamazoo MI'
            try:
                boat_stock = boats.find('li', class_='stockno').find('span', class_='unitValue').text
            except:
                boat_stock = random_numbers
            try:
                boat_msrp = boats.find('span', class_='InvRetPrc').text.replace('Click for Quote!', 'N/A').replace('$','').replace(',','').strip()
            except:
                boat_msrp = 'N/A'
            try:
            ...
            with open(f'DailyFiles/Leadersrpm {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company, boat_model, boat_floor, boat_length, boat_engine, boat_stock, 'Leadersrpm', boat_location, boat_msrp, boat_discount, today])
