...
class riverCity(scrapy.Spider):
    name = 'riverCity'  # Unique identifier for the spider
    
    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.rivercityboatsales.com/default.asp?page=xNewInventory#page=xNewInventory&p=1&vt=boat',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            if (len(boat_info) == 3):
                ...
                boat_floor = boat_floor.replace('®','').replace('™','')
            boat_location = 'Aurora OR'
            try:
                boat_stock = boats.find('li', class_='stockno').find('span', class_='unitValue').text
            except:
                boat_stock = generate_random_numbers()
            try:
                boat_msrp = boats.find('div', class_='dvPriceGroup').find('span', class_='NoBGInvPrice').text.replace('Click for Quote!', 'N/A').replace('$','').replace(',','').strip()
            except:
                boat_msrp = 'N/A'
            try:
            ...
            with open(f'DailyFiles/River City {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company, boat_model, boat_floor, boat_length, boat_engine, boat_stock, 'River City', boat_location, boat_msrp, boat_discount, today])
