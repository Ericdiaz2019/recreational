...
class montanaBoatCenter(scrapy.Spider):
    name = 'montanaBoatCenter'  # Unique identifier for the spider
    
    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.montanaboatcenter.com/default.asp?page=xNewInventory#page=xNewInventory&vt=boat',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            try:
                boat_msrp = boats.find('div', class_='InvPrice').find('span').text.replace('$', '').replace(',', '').replace('Contact for Your Pricing!','N/A').strip()
            except:
                boat_msrp = 'N/A'
            try:
            ...
            with open(f'DailyFiles/MontanaBoatCenter {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company,boat_model,boat_floor,boat_length,boat_engine,boat_stock,'MontanaBoatCenter',boat_location,boat_msrp,boat_discount,today])
