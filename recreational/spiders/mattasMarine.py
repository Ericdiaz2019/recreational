...
class mattasMarine(scrapy.Spider):
    name = 'mattasMarine'  # Unique identifier for the spider
    
    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.mattasmarine.com/new-boats-for-sale--xNewInventory?vt=boat#page=xNewInventory&vt=boat',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            if (len(boat_info) == 3):
                ...
                boat_floor = boat_floor.replace('®','').replace('™','')
            boat_location = 'Junction CO'
            try:
                boat_stock = boats.find('li', class_='stockno').find('span', class_='unitValue').text
            except:
                boat_stock = 'N/A'
            try:
                boat_msrp = boats.find('div', class_='dvPriceGroup').find('span', class_='InvRetPrc').text.replace('Click for Quote!', 'N/A').replace('$','').replace(',','').replace('YEAR END LIQUIDATION SALE','').strip()
            except:
                boat_msrp = 'N/A'
            try:
                boat_discount = boats.find('div', class_='dvPriceGroup').find('span', class_='NoBGInvPrice').text.replace('Click for Quote!', 'N/A').replace('$','').replace(',','').replace('CALL FOR PRICING', 'N/A').replace('YEAR END LIQUIDATION SALE','').strip()
            except:
                boat_discount = 'N/A'
            try:
            ...
            with open(f'DailyFiles/Mattas Marine {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company, boat_model, boat_floor, boat_length, boat_engine, boat_stock, 'Mattas Marine', boat_location, boat_msrp, boat_discount, today])
