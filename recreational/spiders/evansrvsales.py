...
class evansrvsales(scrapy.Spider):
    name = "evansrvsales"


    def start_requests(self):
        web_links = {
            'Travel Trailer': 'https://www.evansrvsales.com/rv-search?s=true&condition=1&types=29&pagesize=12',
            'FifthWheel': 'https://www.evansrvsales.com/rv-search?s=true&condition=1&types=5&pagesize=12',
            'ToyHauler': 'https://www.evansrvsales.com/rv-search?s=true&condition=1&types=26%2C28&pagesize=12',
            'Destination Trailer' : 'https://www.evansrvsales.com/rv-search?s=true&condition=1&types=3&pagesize=12'
        }
    ...
    def parse_units(self, response):
        ...
        for unit in units:
            ...
            unit_stock = num()
            unit_location = 'Petersburg IN' #unit.find('span', class_='unit-location-text').text.replace('.','').replace(',','').replace('""','').strip()
            ...
            with open(f'DailyFiles/EvansRV Sales {today}.csv', 'a', newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:  # If file is empty, write header
                    writer.writerow(['Year','Company','Brand','FloorPlan','Msrp','Discount','Stock-Number','Unit Type','Location','Dealer','Date'])
                
                writer.writerow([unit_year,unit_company,unit_brand,unit_floor,unit_msrp,unit_discount,unit_stock,unit_category,unit_location,'EvansRV Sales',today])
