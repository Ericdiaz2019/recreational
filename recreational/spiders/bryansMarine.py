...
class bryansMarine(scrapy.Spider):
    name = 'bryansMarine'  # Unique identifier for the spider
    
    def start_requests(self):
        # Dictionary of URLs the spider will start with
        urls = { 
            'Pontoon': 'https://www.bryansmarine.com/default.asp?page=xNewInventory#page=xNewInventory&p=1&vt=boat',  # Add more URLs as needed
        }
    ...
    def parse_units(self, response):
        ...
        for boats in units:
            ...
            boat_company = boat_company.replace('®','').replace('™','').replace('® ', '')
            if (len(boat_info) == 3):
                boat_model = 'N/A'
                boat_floor = boat_info[2]
                boat_floor = boat_floor.replace('®','').replace('™','').replace('® ', '')
            else:
                boat_model = boat_info[2]
                boat_model = boat_model.replace('®','').replace('™','').replace('® ', '')
                boat_floor = boat_info[3]
                boat_floor = boat_floor.replace('® ','').replace('® ', '')
            boat_location = 'Vidalia LA'
            ...
            with open(f'DailyFiles/Bryans Marine {today}.csv', 'a',newline='') as file:
                writer = csv.writer(file, quoting=csv.QUOTE_ALL)
                if file.tell() == 0:
                    writer.writerow(['Year', 'Company', 'Model','FloorPlan', 'Length','Engine', 'Stock Number','Dealer', 'Location','MSRP','DISCOUNT','Date'])
                
                writer.writerow([boat_year, boat_company,boat_model,boat_floor,boat_length,boat_engine,boat_stock,'Bryans Marine',boat_location,boat_msrp,boat_discount,today])
